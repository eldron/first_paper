 ...
 */
//hg modify start 2011.4.01
//#define STLC_LIST_POISON1  ((void *) 0x40000000)
//#define STLC_LIST_POISON2  ((void *) 0x040000004)
//now
#define STLC_LIST_POISON1  ((void *) NULL)
#define STLC_LIST_POISON2  ((void *) NULL)
//hg modify end 2011.4.01
 ...
 */
static inline void __stlc_list_add(struct stlc_list_head *new,
 ...
 */
static inline void stlc_list_add(struct stlc_list_head *new, struct stlc_list_head *head)
{
    __stlc_list_add(new, head, head->next);
}

/**
 * stlc_list_add_tail - add a new entry
 * @new: new entry to be added
 ...
 */
static inline void stlc_list_add_tail(struct stlc_list_head *new, struct stlc_list_head *head)
{
    __stlc_list_add(new, head->prev, head);
}
...
/**
 * stlc_list_replace - replace old entry by new one
...
#define stlc_list_for_each_entry_continue(pos, head, member)			\
	for (pos = stlc_list_entry(pos->member.next, typeof(*pos), member),	\
		     prefetch_load(pos->member.next);			\
	     &pos->member != (head);					\
	     pos = stlc_list_entry(pos->member.next, typeof(*pos), member),	\
		     prefetch_load(pos->member.next))
...
#define STLC_INIT_HLIST_HEAD(ptr) do {(ptr)->first = NULL;  (ptr)->lock = 0; barrier();} while(0)
...
{
    __stlc_hlist_del(n);
    n->next = STLC_LIST_POISON1;
    n->pprev = STLC_LIST_POISON2;
}
