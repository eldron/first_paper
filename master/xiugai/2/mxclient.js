var mxClient = {
  VERSION: '1.5.1.7',
	...
	IS_MT : (navigator.userAgent.indexOf('Firefox/') >= 0
			&& navigator.userAgent.indexOf('Firefox/1') < 0 && navigator.userAgent
			.indexOf('Firefox/2') < 0)
			|| (navigator.userAgent.indexOf('Iceweasel/') >= 0
					&& navigator.userAgent.indexOf('Iceweasel/1') < 0 && navigator.userAgent
					.indexOf('Iceweasel/2') < 0)
			|| (navigator.userAgent.indexOf('SeaMonkey/') >= 0 && navigator.userAgent
					.indexOf('SeaMonkey/1') < 0)
			|| (navigator.userAgent.indexOf('Iceape/') >= 0 && navigator.userAgent
					.indexOf('Iceape/1') < 0),
...
if (typeof(mxLanguage) != 'undefined') {
  ...
  mxClient.language = (document.all) ? navigator.userLanguage: navigator.language;
...
var mxUtils = {
  ...
  createImage: function(src) {
    ...
    var imageNode = null;
    if (document.all && imgName.substring(imgName.length - 3, imgName.length).toUpperCase() == 'PNG') {
...
var mxEvent = {
  ...
  addMouseWheelListener: function(funct) {
    if (funct != null) {
      ...
      if (!document.all) {
        var eventName = (!document.all || false) ? 'mousewheel': 'DOMMouseScroll';
...
mxWindow.prototype.init = function(x, y, width, height, style) {
  ...
  if (!document.all && mxClient.WINDOW_SHADOWS) {
  ...
  } else if (document.all && !mxClient.WINDOW_SHADOWS) {
  ...
  if (document.all || this.content.nodeName.toUpperCase() != 'DIV') {
...
mxWindow.prototype.show = function() {
  ...
  this.activate();
  var style = mxUtils.getCurrentStyle(this.contentWrapper);
  if (!document.all && (style.overflow == 'auto' || this.resize != null)) {
    this.contentWrapper.style.height = (this.div.offsetHeight - this.title.offsetHeight - 2) + 'px';
  }
...
mxPopupMenu.prototype.init = function() {
  ...
  this.div.appendChild(this.table);
  if (!document.all && mxClient.MENU_SHADOWS) {
    this.shadow = document.createElement('div');
    this.shadow.className = 'mxPopupMenuShadow';
    this.shadow.style.zIndex = this.zIndex - 1;
    mxUtils.setOpacity(this.shadow, 70);
  } else if (document.all && !mxClient.MENU_SHADOWS) {
    this.div.style.filter = '';
  }
...
mxShape.prototype.configureVmlShape = function(node) {
  ...
  if (color != null && color != mxConstants.NONE) {
    ...
    this.fillNode.setAttribute('color', color);
    if (this.gradient != null) {
      this.fillNode.setAttribute('type', 'gradient');
      this.fillNode.setAttribute('color2', this.gradient);
      var angle = '180';
      ...
      this.fillNode.setAttribute('angle', angle);
    }
    if (this.opacity != null) {
      this.fillNode.setAttribute('opacity', this.opacity + '%');
      if (this.gradient != null) {
        this.fillNode.setAttribute('o:opacity2', this.opacity + '%');
      }
    }
  } else {
  ...
  if (this.strokeNode != null) {
    if (this.strokeNode != null) {
      if (this.isDashed) {
        if (this.strokeNode.getAttribute('dashstyle') != '2 2') {
          this.strokeNode.setAttribute('dashstyle', '2 2');
        }
      } else {
        if (this.strokeNode.getAttribute('dashstyle') != 'solid') {
...
mxShape.prototype.createPoints = function(moveCmd, lineCmd, curveCmd, isRelative) {
  ...
  var points = moveCmd + ' ' + (Math.floor(p0.x - offsetX) + tx) + ' ' + (Math.floor(p0.y - offsetY) + tx) + ' ';
  for (var i = 1; i < this.points.length; i++) {
    ...
    if ((this.isRounded && i < this.points.length - 1) && (dx != 0 || dy != 0) && this.scale > 0.3) {
      ...
      points += lineCmd + ' ' + (Math.floor(pt.x - offsetX) + tx) + ' ' + (Math.floor(pt.y - offsetY) + tx) + ' ';
    }
  }
  return points;
};
  ...
  if (node != null) {
    if (mxUtils.isVml(node)) {
      ...
      if (this.bounds != null && !isNaN(this.bounds.x) && !isNaN(this.bounds.y) && !isNaN(this.bounds.width) && !isNaN(this.bounds.height)) {
        ...
        node.style.left = Math.floor(this.bounds.x - sw / 2) + 'px';
        node.style.top = Math.floor(this.bounds.y - sw / 2) + 'px';
        node.style.width = Math.floor(Math.max(0, this.bounds.width + sw)) + 'px';
        node.style.height = Math.floor(Math.max(0, this.bounds.height + sw)) + 'px';
      }
    }
...
mxShape.prototype.updateVmlShape = function(node) {
  ...
  if (this.bounds != null && !isNaN(this.bounds.x) && !isNaN(this.bounds.y) && !isNaN(this.bounds.width) && !isNaN(this.bounds.height)) {
    ...
    var w = Math.max(0, Math.floor(this.bounds.width));
    var h = Math.max(0, Math.floor(this.bounds.height));
...
mxShape.prototype.updateVmlGlassPane = function() {
  if (this.bounds != null && this.node.nodeName == 'group' && this.style != null && mxUtils.getValue(this.style, mxConstants.STYLE_GLASS, 0) == 1) {
    ...
    var d = 'm ' + ( - sw) + ' ' + ( - sw) + ' l ' + ( - sw) + ' ' + Math.floor(b.height * size) + ' c ' + Math.floor(b.width * 0.3) + ' ' + Math.floor(b.height * 0.6) + ' ' + Math.floor(b.width * 0.7) + ' ' + Math.floor(b.height * 0.6) + ' ' + Math.floor(b.width + sw) + ' ' + Math.floor(b.height * size) + ' l ' + Math.floor(b.width + sw) + ' ' + ( - sw) + ' x e';
    ...
    this.node.glassOverlay.setAttribute('coordsize', Math.floor(this.bounds.width) + ',' + Math.floor(this.bounds.height));
...
var mxMarker = {
  markers: [],
  paintMarker: function(node, type, p0, pe, color, strokewidth, size, scale, x0, y0) {
    ...
    if (marker != null) {
      ...
      if (isVml) {
        ...
        node.setAttribute('stroke', color);
        node.setAttribute('fill', color);
        var op = (this.opacity != null) ? this.opacity / 100 : 1;
...
(function() {
  var tmp = function(node, type, pe, nx, ny, strokewidth, size, scale, isVml) {
    if (isVml) {
      node.setAttribute('path', 'm' + Math.floor(pe.x) + ',' + Math.floor(pe.y) + ' l' + Math.floor(pe.x - nx - ny / 2) + ' ' + Math.floor(pe.y - ny + nx / 2) + ((type != mxConstants.ARROW_CLASSIC) ? '': ' ' + Math.floor(pe.x - nx * 3 / 4) + ' ' + Math.floor(pe.y - ny * 3 / 4)) + ' ' + Math.floor(pe.x + ny / 2 - nx) + ' ' + Math.floor(pe.y - ny - nx / 2) + ' x e');
    } else {
...
mxMarker.markers[mxConstants.ARROW_OPEN] = function(node, type, pe, nx, ny, strokewidth, size, scale, isVml) {
  if (isVml) {
    ...
    node.setAttribute('strokeweight', strokewidth * scale);
  } else {
...
mxMarker.markers[mxConstants.ARROW_OVAL] = function(node, type, pe, nx, ny, strokewidth, size, scale, isVml) {
  ...
  if (isVml) {
    var s = absSize / 2;
    node.setAttribute('path', 'm' + Math.floor(pe.x) + ' ' + Math.floor(pe.y - s) + ' qx' + Math.floor(pe.x + s) + ' ' + Math.floor(pe.y) + ' qy' + Math.floor(pe.x) + ' ' + Math.floor(pe.y + s) + ' qx' + Math.floor(pe.x - s) + ' ' + Math.floor(pe.y) + ' qy' + Math.floor(pe.x) + ' ' + Math.floor(pe.y - s) + ' x e');
  } else {
    node.setAttribute('d', 'M ' + (pe.x - ny / 2) + ' ' + (pe.y + nx / 2) + ' a ' + (absSize / 2) + ' ' + (absSize / 2) + ' 0  1,1 ' + (nx / 8) + ' ' + (ny / 8) + ' z');
  }
...
mxMarker.markers[mxConstants.ARROW_DIAMOND] = function(node, type, pe, nx, ny, strokewidth, size, scale, isVml) {
  if (isVml) {
    node.setAttribute('path', 'm' + Math.floor(pe.x + nx / 2) + ' ' + Math.floor(pe.y + ny / 2) + ' l' + Math.floor(pe.x - ny / 2) + ' ' + Math.floor(pe.y + nx / 2) + ' ' + Math.floor(pe.x - nx / 2) + ' ' + Math.floor(pe.y - ny / 2) + ' ' + Math.floor(pe.x + ny / 2) + ' ' + Math.floor(pe.y - nx / 2) + ' x e');
  } else {
...
mxActor.prototype.redrawVml = function() {
  this.updateVmlShape(this.node);
  this.node.setAttribute('path', this.createPath());
};
...
mxRhombus.prototype.redrawVml = function() {
  ...
  var w = Math.floor(this.bounds.width);
  var h = Math.floor(this.bounds.height);
  this.node.setAttribute('path', 'm ' + Math.floor(x + w / 2) + ' ' + y + ' l ' + (x + w) + ' ' + Math.floor(y + h / 2) + ' l ' + Math.floor(x + w / 2) + ' ' + (y + h) + ' l ' + x + ' ' + Math.floor(y + h / 2) + ' x e');
};
...
mxArrow.prototype.redrawVml = function() {
  if (this.points != null) {
    ...
    this.node.setAttribute('path', 'm' + Math.floor(p0x) + ' ' + Math.floor(p0y) + ' l' + Math.floor(p1x) + ' ' + Math.floor(p1y) + ' ' + Math.floor(p2x) + ' ' + Math.floor(p2y) + ' ' + Math.floor(p3x) + ' ' + Math.floor(p3y) + ' ' + Math.floor(pe.x - spacing * nx) + ' ' + Math.floor(pe.y - spacing * ny) + ' ' + Math.floor(p5x) + ' ' + Math.floor(p5y) + ' ' + Math.floor(p5x + floorx) + ' ' + Math.floor(p5y + floory) + ' ' + Math.floor(p0x) + ' ' + Math.floor(p0y) + ' xe');
  }
};
...
mxText.prototype.create = function(container) {
  ...
  if (this.dialect == mxConstants.DIALECT_SVG) {
    ...
    if (!document.all && !mxClient.NO_FO) {
...
mxText.prototype.redrawForeignObject = function() {
  ...
  if (this.overflow != 'fill') {
    ...
    if (this.horizontal) {
      ...
      var x0 = x + w / 2;
      var y0 = y + h / 2;
      tr = 'scale(' + this.scale + ') rotate(270 ' + x0 + ' ' + y0 + ')';
      x += offset.y;
      y -= offset.x;
    }
    tr += ' translate(' + x + ' ' + y + ')';
...
mxText.prototype.redrawSvg = function() {
  ...
  if (this.value.length > 0 && this.backgroundNode != null && this.node.firstChild != null) {
    ...
    this.backgroundNode.setAttribute('x', this.boundingBox.x + this.scale);
    this.backgroundNode.setAttribute('y', this.boundingBox.y + 3 * this.scale);
...
mxLine.prototype.redrawVml = function() {
  ...
  this.innerNode.setAttribute('coordsize', this.node.getAttribute('coordsize'));
  this.innerNode.setAttribute('strokeweight', this.node.getAttribute('strokeweight'));
  this.innerNode.style.width = this.node.style.width;
  this.innerNode.style.height = this.node.style.height;
  var w = Math.floor(this.bounds.width);
  var h = Math.floor(this.bounds.height);
  if (this.direction == mxConstants.DIRECTION_NORTH || this.direction == mxConstants.DIRECTION_SOUTH) {
    this.innerNode.setAttribute('path', 'm ' + Math.floor(w / 2) + ' 0' + ' l ' + Math.floor(w / 2) + ' ' + h + ' e');
  } else {
    this.innerNode.setAttribute('path', 'm 0 ' + Math.floor(h / 2) + ' l ' + w + ' ' + Math.floor(h / 2) + ' e');
  }
};
...
mxImageShape.prototype.create = function() {
  ...
  if (this.dialect == mxConstants.DIALECT_SVG) {
    ...
    if (this.dialect != mxConstants.DIALECT_VML || this.image.substring(0, 5) == 'DATA:' || this.image.substring(0, 6) == 'MHTML:') {
      node = document.createElement('DIV');
      this.configureHtmlShape(node);
      if (document.all && imgName.substring(imgName.length - 4, imgName.length) == ".PNG") {
        var div = document.createElement('DIV');
        div.style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader (src=\'' + this.image + '\', sizingMethod=\'scale\')';
  ...
  if (this.imageNode != null && this.bounds != null) {
    if (this.preserveImageAspect) {
      function update(w, h) {
        if (w > 0 && h > 0) {
          ...
          var x0 = (this.bounds.width - w * s) / 2;
          var y0 = (this.bounds.height - h * s) / 2;
          var st = this.imageNode.style;
          if (this.imageNode.parentNode == this.node) {
            st.position = 'absolute';
            st.left = Math.floor(x0) + 'px';
            st.top = Math.floor(y0) + 'px';
          } else {
            st.left = Math.floor(this.bounds.x + x0) + 'px';
            st.top = Math.floor(this.bounds.y + y0) + 'px';
          }
      ...
      function() {
        update.call(this, img.width, img.height);
      });
      img.src = this.image;
    }
    ...
    if (flipH || flipV) {
      if (this.imageNode.nodeName == 'DIV') {
        var filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader (src=\'' + this.image + '\', sizingMethod=\'scale\') ' + (((flipH) ? 'flipH() ': '') + ((flipV) ? 'flipV()': ''));
        if (this.imageNode.style.filter != filter) {
          this.imageNode.style.filter = filter;
        }
      }
      if (this.imageNode.nodeName == 'IMG' || mxUtils.isVml(this.imageNode)) {
        var filter = ((flipH) ? 'flipH() ': '') + ((flipV) ? 'flipV()': '');
        if (this.imageNode.style.filter != filter) {
          this.imageNode.style.filter = filter;
        }
      } else {
...
mxLabel.prototype.createHtml = function() {
  ...
  if (this.image != null) {
    ...
    if (this.opacity != null) {
      mxUtils.setOpacity(this.imageNode, this.opacity);
    }
...
mxLabel.prototype.createVml = function() {
  ...
  if (this.image != null) {
    if (this.image.substring(0, 5) == 'data:') {
      ...
      this.imageNode.setAttribute('src', this.image);
      if (this.opacity != null) {
        this.imageNode.style.filter = 'alpha(opacity=' + this.opacity + ')';
      }
...
mxCylinder.prototype.redrawVml = function() {
  ...
  this.updateVmlShape(this.background);
  this.updateVmlShape(this.foreground);
  this.background.setAttribute('path', this.createPath(false));
  this.foreground.setAttribute('path', this.createPath(true));
};
...
mxConnector.prototype.createVml = function() {
  ...
  this.strokeNode = document.createElement('v:stroke');
  this.shapeNode.appendChild(this.strokeNode);
  ...
  if (this.startArrow != null) {
    ...
    this.startStroke.setAttribute('joinstyle', 'miter');
  ...
  if (this.endArrow != null) {
    ...
    this.endStroke.setAttribute('joinstyle', 'miter');
...
mxConnector.prototype.reconfigure = function() {
  ...
  this.shadow = false;
  if (mxUtils.isVml(this.node)) {
    this.shapeNode.style.visibility = 'hidden';
    this.configureVmlShape(this.shapeNode);
    var op = (this.opacity != null) ? this.opacity: '100';
    if (this.start != null) {
      this.startFill.setAttribute('opacity', op + '%');
      this.startStroke.setAttribute('opacity', op + '%');
    }
    if (this.end != null) {
      this.endFill.setAttribute('opacity', op + '%');
      this.endStroke.setAttribute('opacity', op + '%');
    }
    this.shapeNode.style.visibility = 'visible';
  } else {
    mxShape.prototype.reconfigure.apply(this, arguments);
  }
};
mxConnector.prototype.redrawVml = function() {
  if (this.node != null && this.points != null && this.bounds != null && !isNaN(this.bounds.x) && !isNaN(this.bounds.y) && !isNaN(this.bounds.width) && !isNaN(this.bounds.height)) {
    this.updateVmlShape(this.node);
    if (this.start != null) {
      this.start.style.width = this.node.style.width;
      this.start.style.height = this.node.style.height;
      this.start.setAttribute('coordsize', this.node.getAttribute('coordsize'));
    ...
    if (this.end != null) {
      this.end.style.width = this.node.style.width;
      this.end.style.height = this.node.style.height;
      this.end.setAttribute('coordsize', this.node.getAttribute('coordsize'));
...
mxConnector.prototype.redrawMarker = function(node, type, p0, pe, color, size) {
  return mxMarker.paintMarker(node, type, p0, pe, color, this.strokewidth, size, this.scale, this.bounds.x, this.bounds.y);
};
...
mxCoordinateAssignment.prototype.setEdgePosition = function(cell) {
  ...
  if (cell.temp[0] != 101207) {
    for (var i = 0; i < cell.edges.length; i++) {
      ...
      for (var j = loopStart; j != loopLimit; j += loopDelta) {
...
mxCellRenderer.prototype.installListeners = function(state) {
  ...
  var getState = function(evt) {
    var result = state;
    if (document.all || mxClient.IS_TOUCH) {
      var x = mxEvent.getClientX(evt);
      var y = mxEvent.getClientY(evt);
      var pt = mxUtils.convertPoint(graph.container, x, y);
      result = graph.view.getState(graph.getCellAt(pt.x, pt.y));
...
function mxGraph(container, model, renderHint) {
  ...
  } else if (renderHint == mxConstants.RENDERING_HINT_EXACT && document.all) {
...
mxCellOverlay.prototype.getBounds = function(state) {
  ...
  return new mxRectangle(pt.x - w * this.defaultOverlap * s + this.offset.x, pt.y - h * this.defaultOverlap * s + this.offset.y, w * s, h * s);
};
...
mxConnectionHandler.prototype.mouseDown = function(sender, me) {
  if (this.isEnabled() && this.graph.isEnabled() && !me.isConsumed() && !this.isConnecting() && !this.graph.isForceMarqueeEvent(me.getEvent()) && this.constraintHandler.currentFocus != null && this.constraintHandler.currentConstraint != null) {
    ...
    me.consume();
  } else if (this.isEnabled() && this.graph.isEnabled() && !me.isConsumed() && !this.isConnecting() && !this.graph.isForceMarqueeEvent(me.getEvent()) && this.previous != null && this.error == null && (this.icons == null || (this.icons != null && this.icon != null))) {
    this.first = mxUtils.convertPoint(this.graph.container, me.getX(), me.getY());
    this.edgeState = this.createEdgeState(me);
    if (this.waypointsEnabled && this.previous != null && this.start != null && this.shape == null) {
      this.waypoints = null;
    ...
    if (this.previous != null) {
      ...
      window.setTimeout(mxUtils.bind(this, handler), this.tapAndHoldDelay);
    }
  }
    ...
    if (this.previous != null && this.first != null) {
      ...
      if (this.selectedIcon != null) {
        ...
        if (state != null && this.targetConnectImage) {
          ...
          var bounds = new mxRectangle(pt.x + this.connectIconOffset.x, pt.y + this.connectIconOffset.y, w, h);
          this.selectedIcon.bounds = bounds;
        }
        this.selectedIcon.redraw();
      }
      if (this.edgeState != null) {
        this.edgeState.absolutePoints = [null, (state != null) ? null: current];
        this.graph.view.updateFixedTerminalPoint(this.edgeState, this.previous, true, this.graph.getConnectionConstraint(this.edgeState, this.previous, true));
        if (state != null) {
          this.graph.view.updateFixedTerminalPoints(this.edgeState, this.previous, state);
        }
        this.graph.view.updatePoints(this.edgeState, this.waypoints, this.previous, state);
        this.graph.view.updateFloatingTerminalPoints(this.edgeState, this.previous, state);
        current = this.edgeState.absolutePoints[this.edgeState.absolutePoints.length - 1];
        pt2 = this.edgeState.absolutePoints[0];
      } else {
        if (state != null) {
          if (this.constraintHandler.currentConstraint == null) {
            var tmp = this.getTargetPerimeterPoint(state, me);
            if (tmp != null) {
              current = tmp;
            }
          } else {
      ...
      if (edge != null) {
        ...
        if (this.waypoints != null && this.waypoints.length > 0) {
          ...
          for (var i = 0; i < this.waypoints.length; i++) {
            ...
            geo.points.push(new mxPoint(pt.x / s, pt.y / s));
          }
        }
        if (target == null) {
...
mxEdgeHandler.prototype.mouseDown = function(sender, me) {
  var handle = null;
  if (!document.all || me.getState() == this.state) {
...
mxTooltipHandler.prototype.init = function() {
  if (document.body != null) {
    ...
    this.div.style.visibility = 'hidden';
    this.div.style.zIndex = this.zIndex;
    if (!document.all && mxClient.TOOLTIP_SHADOWS) {
      this.shadow = document.createElement('div');
      ...
      mxUtils.setOpacity(this.shadow, 70);
      document.body.appendChild(this.shadow);
    } else if (document.all && !mxClient.TOOLTIP_SHADOWS) {
      this.div.style.filter = '';
    }
    document.body.appendChild(this.div);
...
mxCodecRegistry.register(function() {
  ...
  codec.afterEncode = function(enc, obj, node) {
    if (obj.value != null && obj.value.nodeType == mxConstants.NODETYPE_ELEMENT) {
      var tmp = node;
      node = (document.all) ? obj.value.cloneNode(true) : enc.document.importNode(obj.value, true);
